function drawmatrix_atlas(mat,varargin)
...
%
% drawmatrix_atlas(subnet_scandur{1},'subnetwork')
...
numatlasnodes = p.Results.numatlasnodes;
...
if size(mat,1) ~= size(mat,2)
    ...
    mat=reorder_matrix_atlas(mat,atlascategory);
    if ~reorderimg
...
% check atlas size
load(sprintf('map%0.0f_%s.mat',numatlasnodes,atlascategory));
if matdim==numatlasnodes; usingsummary=0;
elseif matdim==length(unique(map.category))
    usingsummary=1;
else
    error(sprintf('Must provide either full %0.0f node atlas or summary',numatlasnodes))
end
...
if strcmp(datacategory,'none')
    imagesc(mat*imscale)
elseif strcmp(datacategory,'none_noscale')
    image(mat*imscale)
...
cb=colorbar;
if ~(strcmp(datacategory,'none') | strcmp(datacategory,'none_noscale'))
