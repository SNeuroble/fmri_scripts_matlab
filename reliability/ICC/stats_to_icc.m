...
% computes 1, 2- or 3-factor ICC (detects from nfactors in stats) 
% need subject specifications from ftbl
...
% remember, for TRT ICC: DSrange=6 and Dstepsz=1 means 1:6 runs (stepsz=1 run)

tempsave=0; % turn on to save ICC if running directly, not within ICC script
saveimg=0; % turn on to save detailed ICC img

if mod(DSrange, Dstepsz) ~= 0
    error('D-Study is misspecified.') % want Dstepsz <= 1 and DSrange divisible by it
end
...
% detailed_icc_G=[]; % SMN
% detailed_icc_D=[]; % SMN
detailed_icc_G{1}=[]; % SMN
detailed_icc_D{1}=[]; % SMN
if nvarcomp==2
...
%         cutoff=10;
        if it1 < cutoff %new
%             detailed_icc_G{it1)=icc_G; % SMN
%             detailed_icc_D(it1)=icc_D; % SMN
            detailed_icc_G{it1}=icc_G; % SMN
            detailed_icc_D{it1}=icc_D; % SMN
        end
...
%                         icc_D{ref_factor}(i,j)=var_comp{a}(i,j)/(var_comp{a}(i,j)+var_comp{b}(i,j)/factor_size(b)+var_comp{c}(i,j)/factor_size(c)+var_comp{d}(i,j)/factor_size(b)+var_comp{e}(i,j)/factor_size(c)+var_comp{f}(i,j)/(factor_size(b)*factor_size(c))+var_comp{g}(i,j)/(factor_size(b)*factor_size(c)));
                ...
                Dstudy_G(it1,it2)=nanmean(icc_G{ref_factor}(:)); % half new
                Dstudy_D(it1,it2)=nanmean(icc_D{ref_factor}(:)); % half new
...
%                 if it1<=detailedlim/Dstepsz && it2==1 %new
                if it1==1 && it2<= DSrange/Dstepsz %new
%                     detailed_icc_G(it1,it2)=icc_G; %SMN
%                     detailed_icc_D(it1,it2)=icc_D; %SMN
    ...
    it1=1;
    for n1=n1_all % factor 1
        it2=1;
        for n2=n2_all % factor 2
            it3=1;
            for n3=n3_all % factor 3
                ...
                for i=1:size(var_comp{1},1)
                    for j=1:size(var_comp{1},2)
                        ...
                        % note that this will be skipped if <12 var comp (e.g., from linear model)
                ...
                if it1==1 && it2 <= it2_lim && it3 <= it3_lim %new
%                     detailed_icc_G(it1,it2)=icc_G; % SMN
%                     detailed_icc_D(it1,it2)=icc_D; % SMN
                    detailed_icc_G{it1,it2}=icc_G; % SMN
                    detailed_icc_D{it1,it2}=icc_D; % SMN
...
if nvarcomp<=7
    ...
    for i=1:2 % G-coeff plot, D-coeff plot
        ...
        % to do nonthresholded full colormap, do:
        % figure; image((icc{2,2}-0.4)*159); ax=gca
        % axis square
        % then run the nvarcomp==7 part, making sure to
        %   1. remove {} after icc
        %   2. define n1_all=1:.1:6, n2_all=n1_all, and DSrange=6
